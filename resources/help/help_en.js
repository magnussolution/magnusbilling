Help.load({
    ...
    * Outside of the time range
    ...
    'campaign.type': `Choose Voice, SMS, or WhatsApp.||For WhatsApp, MagnusBilling uses the official Meta WhatsApp Business Cloud API and sends an approved template configured in the Messages tab. MagnusBilling only provides the integration; Meta processes and delivers the message. The account owner is responsible for recipient consent, content, templates, charges, and compliance with all applicable laws and Meta/WhatsApp policies.`,
    'campaign.whatsapp_template_name': `Exact name of an approved WhatsApp template in WhatsApp Manager. The current integration sends templates without variable components, so use a template that does not require parameters.`,
    'campaign.whatsapp_template_language': `Exact language code approved for the WhatsApp template, for example en_US or pt_BR. The name and language must match the template in WhatsApp Manager.`,
    ...
    *Personalized, you may execute any valid option via Asterisk's DIAL command. Example: PJSIP/sipaccount,45,tTr.`,
...
exten => s-CHANUNAVAIL,2,Dial(PJSIP/sipaccount2,,T)
...
You can use the button "process" to reactivate the numbers with pending status.`,
    ...
    'queue.max_wait_time_action': `SipAccount, IVR, QUEUE or LOCAL channel to send the caller if the maximum wait time is reached. Use: PJSIP/sip_account, QUEUE/queue_name, IVR/ivr_name OR LOCAL/extension@context.`,
...
* Custom, it's possible to execute any valid option of the DIAL command of Asterisk, example: PJSIP/contaSIP,45,tTr
