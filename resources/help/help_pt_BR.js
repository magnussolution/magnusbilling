...
Para funcionar é necessário deixar o campo Fromuser no tronco em branco.`,
    ...
    'campaign.type': `Selecione entre Voz, SMS ou WhatsApp.||Para WhatsApp, o MagnusBilling usa a API oficial WhatsApp Business Cloud da Meta e envia um template aprovado configurado na aba Mensagens. O MagnusBilling fornece somente a integração; a Meta processa e entrega a mensagem. O titular da conta é responsável pelo consentimento dos destinatários, conteúdo, templates, cobranças e cumprimento das leis e políticas aplicáveis da Meta/WhatsApp.`,
    'campaign.whatsapp_template_name': `Nome exato de um template do WhatsApp aprovado no WhatsApp Manager. A integração atual envia templates sem componentes variáveis; portanto, use um template que não exija parâmetros.`,
    'campaign.whatsapp_template_language': `Código de idioma exato aprovado para o template do WhatsApp, por exemplo en_US ou pt_BR. O nome e o idioma devem corresponder ao template cadastrado no WhatsApp Manager.`,
...
* Personalizado, então é possível a execução de qualquer opção válida do comando DIAL do asterisk, exemplo: PJSIP/contaSIP,45,tTr.`,
...
exten => s-CHANUNAVAIL,2,Dial(PJSIP/contavoip2,,T)
...
Você pode usar o botão processar para reativar os números que estão com status pendente.`,
    ...
    'queue.max_wait_time_action': `Coloque uma Conta VoIP, IVR, Fila de espera ou um canal LOCAL para enviar o cliente caso o tempo máximo de espera for superado. Formatos aceitos: PJSIP/conta_voip, QUEUE/nome_da_queue, IRV/nome_da_ivr ou LOCAL/extension@context.`,
...
* Grupo, o nome do grupo colocado aqui, deve ser exatamente o mesmo do grupo das contas SIP que deseja receber as chamadas, vai chamar todas as contas SIP do grupo. 
* Personalizado, é possível a execução de qualquer opção válida do comando DIAL do Asterisk, exemplo: PJSIP/contaSIP,45,tTr
